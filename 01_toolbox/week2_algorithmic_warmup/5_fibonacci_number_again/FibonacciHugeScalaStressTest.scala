...
object FibonacciHugeScalaStressTest {
  ...
  type FibonacciHugeStrategy = (Long, Long) => Long

  // Method to compare strategies
  def compareStrategies(strategy1: FibonacciHugeStrategy, strategy2: FibonacciHugeStrategy, n: Long, m: Long): Boolean = {
    val result1 = strategy1(n, m)
    val result2 = strategy2(n, m)
    if (result1 != result2) {
  ...
  def stressTest(maxIterations: Int, maxN: Int): Unit = {
    ...
    for (iteration <- 1 to maxIterations) {
      val n = random.nextInt(maxN) + 1L
      val m = random.nextInt(maxN) + 2L

      println(s"Testing with n = $n and m = $m")

      val strategies = Seq(
        getFibonacciHugeNaive1 _,
        getFibonacciHugeNaive2 _,
        getFibonacciHugeNaive3 _,
        getFibonacciHuge _,
        getFibonacciHugeNaive1Functional _,
        getFibonacciHugeNaive2Functional _,
        getFibonacciHugeFunctional _
      )

      val results = strategies.map(strategy => strategy(n, m))
      if (results.distinct.size != 1) {
        println(s"Testing with n = $n and m = $m")
        println(s"Discrepancy found: ${results.mkString(", ")}")
      } else {
        println(s"Testing with n = $n and m = $m: OK")
      }
  ...
  def main(args: Array[String]): Unit = {
    stressTest(1000, 40) // Run the stress test

    // Uncomment to run with user input
    //    val n = readLong()
    //    val m = readLong()
    //    println(getFibonacciHugeFunctional(n, m))
  }
